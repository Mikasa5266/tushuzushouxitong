<template>
  <div class="page-layout">
      ...
      <div class="search-container">
        ...
        <div class="search-stats" v-if="WhatSearch || tempDS.length > 0">
            <div class="stat-item">
                ...
                <span class="value">{{ tempDS.length }}</span> 本
            </div>
            <div class="stat-item available">
                <span class="label">✅ 当前可用:</span>
                <span class="value">{{ availableCount }}</span> 本
            </div>
            <div class="stat-item rented">
                <span class="label">❌ 已租/售:</span>
                <span class="value">{{ tempDS.length - availableCount }}</span> 本
            </div>
        </div>
      </div>
        ...
        <template #bodyCell="{ column, text, record }">
          ...
          <template v-if="editableData[record.key]">
            ...
            <div v-else-if="column.dataIndex === 'bookDetails'">
               <Input.TextArea 
                 v-model:value="editableData[record.key]!.bookDetails" 
            ...
            <div v-else-if="column.dataIndex === 'edit'" class="editable-row-operations">
                <span v-if="savingKey === record.key" style="color: #1890ff;">
                    <span class="loading-dots">保存中</span>
                </span>
                <span v-else>
          ...
          <template v-else>
            <template v-if="column.dataIndex === 'bookDetails'">
              <!-- 点击查看详情弹窗 -->
              <a @click="openModal(record.bookDetails)" class="action-link" style="margin-right: 5px;">查看</a>
              <!-- 截取部分显示 -->
            ...
            <template v-else-if="column.dataIndex === 'edit'">
              <div class="editable-row-operations">
                <span>
                  ...
                  <Popconfirm
                        title="确定要删除这本书吗？此操作不可逆。"
                        ok-text="确定"
                        cancel-text="取消"
                        @confirm="deleteBook(record.key)"
                    >
    ...
    <Modal title="图书详细" :footer="null" v-model:open="open" centered>
      <div style="padding: 20px; line-height: 1.6; color: #555; white-space: pre-wrap;">
        {{ currentDetails || '暂无简介' }}
      </div>
    </Modal>
    ...
    <!-- 新增书籍弹窗 -->
    <Modal
      v-model:open="addBookModalVisible"
      title="➕ 批量入库新书"
      @ok="handleAddBook"
      @cancel="addBookModalVisible = false"
      centered
      width="600px"
      okText="确认添加"
      cancelText="取消"
    >
      <Form layout="vertical" class="add-form">
        <div class="form-row">
            <FormItem label="书籍名称" required style="flex: 2">
                <Input v-model:value="newBook.bookName" placeholder="输入书名" />
            </FormItem>
            <FormItem label="作者" required style="flex: 1">
                <Input v-model:value="newBook.bookAuthor" placeholder="输入作者" />
            </FormItem>
        </div>
        <div class="form-row">
            <FormItem label="类型" required>
                <Select v-model:value="newBook.bookKindCode" placeholder="选择类型" style="width: 100%">
                    <SelectOption v-for="(code, name) in kindMap" :key="code" :value="code">
                        {{ name }}
                    </SelectOption>
                </Select>
            </FormItem>
            <FormItem label="单价" required>
                <Input v-model:value="newBook.bookPrice" type="number" prefix="￥" />
            </FormItem>
        ...
        <FormItem label="简介">
            <Input.TextArea v-model:value="newBook.bookDetails" :rows="3" placeholder="输入简介 (50字以内)..." />
        </FormItem>
      </Form>
    </Modal>
  </div>
...
const kindMap = reactive<Record<string, string>>({
    "小说": "FIC", "文学": "LIT", "人文社科": "HUM", "历史": "HIS",
    "哲学宗教": "PHI", "艺术": "ART", "传记": "BIO", "经济": "ECO",
    "金融投资": "FIN", "管理": "MAN", "市场营销": "MAR", "计算机": "IT",
    "自然科学": "SCI", "工程技术": "ENG", "医学": "MED", "教育": "EDU",
    "儿童绘本": "CHI", "青少年读物": "YOU", "生活家具": "LIF", "旅游地图": "TRA",
});
...
const availableCount = computed(() => {
    return tempDS.value.filter(item => item.bookStatus === '空闲').length;
});
...
const openAddBookModal = () => {
    newBook.bookName = '';
    newBook.bookAuthor = '';
    newBook.bookKindCode = undefined;
    newBook.bookPrice = '';
    newBook.bookDetails = '';
    newBook.bookCount = 1;
...
const handleAddBook = async () => {
    if (!newBook.bookName || !newBook.bookKindCode || !newBook.bookPrice || !newBook.bookAuthor) {
...
const fetchData = async () => {
  ...
  try {
    ...
    if (Object.keys(realTypes).length > 0) {
        Object.assign(kindMap, realTypes);
    }
...
const columns = [
  ...
  { title: '简介', dataIndex: 'bookDetails', key: 'bookDetails', width: 150 }, // 宽度增加到 150
